...
const API_BASE = resolveApiBase(
  ...
  qbankWorkflowBase('standard'),
  qbankWorkflowBase('standard')
);
const apiAssetUrl = (pathOrUrl) => qbankApiAssetUrl(API_BASE, pathOrUrl);
const WORKSPACE_BACKUP_KEY = 'qbank_lb_standard_workspace_backup_v1';
...
function ReviewModal({ isOpen, initialRowId, data, onClose, onUpdate, jobId, onImagePreview, onTriggerScanner, insertRow, deleteRow, flaggedIds }) {
  ...
  useEffect(() => {
    ...
    // FIX 2d: currentIndex and currentRow removed from deps — use ref instead
...
export default function Parser() {
  ...
  // FIX 1a: refs that always hold latest state — used in fetchFinalData to avoid stale closures
  ...
  useEffect(() => {
    const loadWorkspace = async () => {
      try {
        ...
        const workspaceToRestore = shouldUseWorkspaceBackup(remoteWorkspace, backupWorkspace, 'dp')
          ? backupWorkspace
          : remoteWorkspace;

        if (workspaceToRestore && Object.keys(workspaceToRestore).length > 0) {
          const backupRows = workspaceRowCount(backupWorkspace || {}, 'dp');
          const remoteRows = workspaceRowCount(remoteWorkspace || {}, 'dp');
          ...
          const { dp_matrices, dp_openTabs, dp_activeTab, dp_editedCells, dp_engineJobId, dp_customCols } = workspaceToRestore;
  ...
  // FIX 1b: keep refs in sync — fetchFinalData reads from these refs, not stale closure
  useEffect(() => {
    ...
    const saveTimer = setTimeout(async () => {
      try {
        ...
        const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceData, 'dp');
        if (backupSave.skipped) {
          const skipKey = `${workspaceRowCount(workspaceData, 'dp')}:${backupSave.backupRows}`;
  ...
  useEffect(() => {
    const iv = setInterval(async () => {
      ...
      try {
        ...
        clearExportQueue(); showToast('✅ Queued export succeeded!');
      } catch { }
    }, 30000);
  ...
  const importRowsToExtractionReview = async (jobId, rows) => {
    ...
    try {
      ...
      const response = await importExtractionRowsToQBankReview(axios, {
        ...
        jobId,
        setName: 'ALL',
        sourceFileName: firstRow['NIOS Filename'] || 'Extraction DataParser',
    ...
    try {
      ...
      eventSourceRef.current.onmessage = (event) => {
        ...
        if (log.type === "system_control" && (log.text === "ALL_DONE" || log.text === "SET_COMPLETE")) {
          ...
          // FIX 1c: fetchFinalData called here — uses refs internally to avoid stale closure
  ...
  // FIX 1d: fetchFinalData now reads from refs — never uses stale closure state
  const fetchFinalData = async (id, queueIdx, options = {}) => {
    try {
      ...
      const nextActiveTab = switchToTab && serverData.length > 0 && (activeTab === 'engine' || !currentHasRows)
        ...
        : activeTab;
      const workspaceData = {
        dp_activeTab: nextActiveTab,
        dp_openTabs: newOpenTabs,
        dp_editedCells: editedCells,
        dp_engineJobId: id,
        dp_customCols: customColumns,
        dp_matrices: newMatrices
      };
      const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceData, 'dp');
      if (!backupSave.skipped) {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      showToast('⚠️ Backend unreachable. Export queued for retry.');
        ...
        <div className="flex items-center gap-3 shrink-0 ml-4">
          <button onClick={async () => {
            ...
            try {
              ...
              const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceSnap, 'dp');
              if (!backupSave.skipped) {
