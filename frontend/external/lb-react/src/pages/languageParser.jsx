...
import {
  ...
  ChevronLeft, ChevronRight, List, Globe, MoreHorizontal
} from 'lucide-react';
...
import { installQBankAxiosAuth, qbankApiAssetUrl, qbankFetch, qbankWorkflowBase } from '../lib/qbankApi';
import { buildExtractionReviewImportSignature, hasExtractionReviewableRows, importExtractionRowsToQBankReview } from '../lib/qbankReviewImport';
import { clearWorkspaceBackup, readWorkspaceBackup, saveWorkspaceBackup, shouldUseWorkspaceBackup, workspaceRowCount } from '../lib/qbankWorkspaceBackup';
...
const API_BASE = resolveApiBase(
  import.meta.env.VITE_LANGUAGE_API_BASE,
  qbankWorkflowBase('language'),
  qbankWorkflowBase('language')
);
const apiAssetUrl = (pathOrUrl) => qbankApiAssetUrl(API_BASE, pathOrUrl);
const WORKSPACE_BACKUP_KEY = 'qbank_lb_language_workspace_backup_v1';
...
const QUESTION_TYPES = [
  "1 Mark (MCQ)", "Objective Type Questions", "Very Short Answer (VSA)",
  "Short Answer (SA)", "Long Answer Type (LA)", "Skill (Map)"
];

const BLOOM_TAXONOMY_OPTIONS = ['Knowledge', 'Understanding', 'Application'];

const INITIAL_INSTRUCTIONS = [
  { type: 'system_control', text: 'LANGUAGE ENGINE READY: Awaiting Document Ingestion.' },
  { type: 'info', text: '1. Select target language and load QP + MS PDFs.' },
  { type: 'info', text: '2. Execute "Run Queue" to begin language-aware extraction.' }
];

// ✅ PATCH 2: Language → script/direction mapping for the /scan-text endpoint.
// Internal config — never shown to the user.
const DOC_LANG_CONFIG = {
  TAMIL:     { language: 'tamil',     script: 'tamil',      direction: 'ltr' },
  TELUGU:    { language: 'telugu',    script: 'telugu',     direction: 'ltr' },
  HINDI:     { language: 'hindi',     script: 'devanagari', direction: 'ltr' },
  MARATHI:   { language: 'marathi',   script: 'devanagari', direction: 'ltr' },
  SANSKRIT:  { language: 'sanskrit',  script: 'devanagari', direction: 'ltr' },
  BENGALI:   { language: 'bengali',   script: 'bengali',    direction: 'ltr' },
  ASSAMESE:  { language: 'assamese',  script: 'bengali',    direction: 'ltr' },
  KANNADA:   { language: 'kannada',   script: 'kannada',    direction: 'ltr' },
  MALAYALAM: { language: 'malayalam', script: 'malayalam',  direction: 'ltr' },
  GUJARATI:  { language: 'gujarati',  script: 'gujarati',   direction: 'ltr' },
  ODIYA:     { language: 'odia',      script: 'odia',       direction: 'ltr' },
  PUNJABI:   { language: 'punjabi',   script: 'gurmukhi',   direction: 'ltr' },
  URDU:      { language: 'urdu',      script: 'arabic',     direction: 'rtl' },
};
...
const getAccurateImageUrl = (rowData, source, pageNum, jobId) => {
  ...
  return apiAssetUrl(`/workspace/${jobId}/${folder}/${baseName}/${prefix}${pageNum}.png`);
};
  ...
  return (
    <tr className={rowClass} onClick={() => onSelect(row.id)}>
      ...
      <td className="p-4 text-right w-24">
        <button className="text-blue-600 font-bold text-xs bg-white border border-blue-200 shadow-sm hover:bg-blue-600 hover:text-white px-3 py-1.5 rounded transition-colors">Edit &rarr;</button>
      </td>
    </tr>
  );
});
...
const ImagePreviewModal = ({ isOpen, imageUrl, jobId, baseName, onClose }) => {
  ...
  const fullPath = apiAssetUrl(`/workspace/${jobId}/images/${safeBaseName}/${imageUrl}?t=${Date.now()}`);
  return (
...
const TextScannerModal = ({ isOpen, onClose, rowData, columnToUpdate, jobId, onScanComplete }) => {
  ...
  // ✅ PATCH 3: handleScan now calls the backend /scan-text endpoint instead of Tesseract.
  // The backend handles regional script OCR accurately for all 13 languages.
  const handleScan = async () => {
    ...
    try {
      ...
      const subjectKey = (rowData['Subject Name'] || '').toUpperCase().trim();
      const cfg = DOC_LANG_CONFIG[subjectKey]
                || { language: 'regional', script: 'auto', direction: 'ltr' };
      canvas.toBlob(async (blob) => {
        ...
        form.append('direction', cfg.direction);
        form.append('script',    cfg.script);
        const res = await qbankFetch(`${API_BASE}/scan-text`, { method: 'POST', body: form });
      ...
      <div className="bg-[#0f172a] rounded-xl shadow-2xl border border-cyan-700 w-full max-w-6xl flex flex-col max-h-[95vh]">
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-700 bg-slate-900/50 flex-shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div className="bg-rose-500/80 text-white text-[11px] font-black px-3 py-1 rounded uppercase tracking-widest">Targeting: Q{rowData['Sl.No']}</div>
        ...
        <div className="flex-1 overflow-auto bg-black/20 p-6 flex justify-center items-start min-h-0 custom-scrollbar shadow-inner">
          {!imgError
            ? <div style={{ width: `${1000 * (zoom / 100)}px` }}>
              <ReactCrop crop={crop} onChange={(_, pc) => setCrop(pc)} onComplete={(c) => setCompletedCrop(c)} className="shadow-2xl rounded bg-white w-full block border border-cyan-600">
                <img ref={imgRef} src={currentImageUrl} onError={() => setImgError(true)} alt="Document" className="w-full h-auto block pointer-events-none select-none" crossOrigin="anonymous" />
              </ReactCrop>
            </div>
...
const ManualCropModal = ({ isOpen, onClose, rowData, columnToUpdate, jobId, onCropComplete }) => {
  ...
  const handleUploadCrop = async () => {
    ...
    } catch { alert("Core processor failed to slice coordinates."); setIsUploading(false); }
  };
  return (
      ...
      <div className="bg-[#0f172a] rounded-xl shadow-2xl border border-slate-700 w-full max-w-6xl flex flex-col max-h-[95vh]">
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-700 bg-slate-900/50 flex-shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div className="bg-emerald-500/80 text-white text-[11px] font-black px-3 py-1 rounded uppercase tracking-widest">Targeting: Q{rowData['Sl.No']}</div>
            <div className="inline-flex items-center gap-2 bg-black/40 border border-slate-700 p-1 rounded-lg">
              <button onClick={() => setCropSource('QP')} className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${cropSource === 'QP' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}>QP</button>
              <button onClick={() => setCropSource('MS')} className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${cropSource === 'MS' ? 'bg-fuchsia-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}>MS</button>
            </div>
...
function VerifyDashboard({ isOpen, onClose, openModalAtId, activeData }) {
  ...
  const errors = useMemo(() => {
    ...
    activeData.forEach((row) => {
      ...
      if (numOpts > 0 && !hasCorrect) critical.push({ id: row.id, qno: row['Sl.No'], message: 'No correct option selected' });
      const qTextLower = qText.toLowerCase();
      if (!(row['If Question is Image, Specify Image Name']) && (qTextLower.includes('figure') || qTextLower.includes('diagram')))
        warnings.push({ id: row.id, qno: row['Sl.No'], message: "Text mentions 'figure/diagram', missing crop" });
      if (row['Duplicate_Flag'] === 'Yes') duplicates.push({ id: row.id, qno: row['Sl.No'], message: 'Flagged as duplicate' });
      const conf = parseInt(row['Extraction_Confidence'] || '75');
      if (conf < 60) lowConf.push({ id: row.id, qno: row['Sl.No'], message: `Low confidence: ${conf}` });
    });
    const existingQNos = activeData.map(r => { const m = String(r['Sl.No'] || '').match(/^(\d+)/); return m ? parseInt(m[1], 10) : NaN; }).filter(n => !isNaN(n));
    if (existingQNos.length > 0) {
      ...
      for (let i = 0; i < uniqueQNos.length - 1; i++)
        for (let j = uniqueQNos[i] + 1; j < uniqueQNos[i + 1]; j++)
          missing.push({ id: null, qno: j, message: `Missing Question ${j}` });
    }
  ...
  return (
    <div onClick={e => e.stopPropagation()} className={`fixed inset-y-0 right-0 w-96 bg-white/95 backdrop-blur-2xl border-l border-slate-200 shadow-[0_0_50px_rgba(0,0,0,0.2)] z-[60] transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
        <h2 className="text-lg font-bold text-slate-800 flex items-center"><ShieldCheck className="mr-2 h-5 w-5 text-violet-600" /> Verification</h2>
...
const DocumentPageImage = React.memo(({ src, alt }) => {
  ...
  if (hasError) {
    return (
      <div className="relative z-10 flex min-h-80 w-full flex-col items-center justify-center gap-2 bg-slate-950 px-6 text-center text-slate-400">
        <AlertTriangle className="h-8 w-8 text-amber-400" />
        <p className="text-sm font-bold text-slate-200">Document page could not load.</p>
        <p className="max-w-md text-xs leading-5">Refresh after the workflow finishes generating page images.</p>
      </div>
    );
  }
...
const PageList = React.memo(({ availablePages, cropSource, currentRow, jobId }) => {
  if (!availablePages.length) {
    return (
      <div className="flex min-h-80 w-full flex-col items-center justify-center gap-2 px-6 text-center text-slate-500">
        <AlertTriangle className="h-8 w-8 text-amber-400" />
        <p className="text-sm font-bold text-slate-300">Source pages are not available yet.</p>
        <p className="max-w-md text-xs leading-5">Run or refresh the extraction so generated page images can be loaded here.</p>
      </div>
    );
  }
      ...
      {availablePages.map((pNum) => {
        ...
        return (
          <div key={`${cropSource}-${pNum}`} id={`doc-page-${pNum}`}
            ...
            <DocumentPageImage src={imgUrl} alt={`Page ${pNum}`} />
          </div>
        );
      })}
    </div>
      ...
      <div className="bg-[#0f172a]/95 border border-slate-700 w-full max-w-7xl h-[95vh] rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-fade-in">
        <div className={`px-4 py-2 border-b flex-shrink-0 ${isVerified ? 'border-emerald-700 bg-emerald-950/30' : 'border-slate-800 bg-slate-900/50'}`}>
          <div className="flex items-center justify-between gap-2 mb-2">
            <div className="flex items-center gap-2 min-w-0 flex-wrap">
              ...
              {isFlagged && <span className="text-xs font-bold text-rose-400 bg-rose-950/50 border border-rose-700 px-2 py-1 rounded shrink-0">⚠ Flagged</span>}
        ...
        <div className="flex flex-1 overflow-hidden bg-[#0B1120]/50">
          <div className="w-7/12 p-5 border-r border-slate-800/80 flex flex-col bg-slate-900/20 relative group">
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs text-slate-500 uppercase tracking-widest font-semibold flex items-center">
                <span className="w-2 h-2 rounded-full bg-violet-500 mr-2"></span>Language Document Source
              </div>
              <div className="flex items-center gap-2 bg-black/40 border border-slate-700 p-1 rounded-lg">
                ...
                <button onClick={() => setCropSource('QP')} className={`px-2 py-1 text-xs font-bold rounded-md border transition-colors ${cropSource === 'QP' ? 'bg-slate-700 text-violet-400 border-violet-500/50' : 'text-slate-500 border-transparent hover:bg-slate-800'}`}>QP</button>
            ...
            <div className="flex-1 rounded-xl bg-slate-900/50 border border-slate-800/80 overflow-y-auto overflow-x-hidden shadow-inner relative custom-scrollbar scroll-smooth" id="pdf-scroll-container">
              <div style={{ width: `${previewZoom}%`, margin: '0 auto' }}>
          ...
          <div ref={rightPanelRef} className="w-5/12 p-7 overflow-y-auto bg-transparent flex flex-col custom-scrollbar gap-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-1 bg-violet-900/30 px-2 py-1 rounded-md border border-violet-800/50 font-mono text-sm font-bold text-violet-400">
                  <span>Q</span>
                  <DebouncedWorkspaceInput value={currentRow['Sl.No']} onChange={(val) => handleFieldUpdate('Sl.No', val)} className="w-10 bg-transparent text-center border-b border-transparent hover:border-violet-500 focus:border-violet-400 outline-none text-violet-300" />
                  <span className="text-slate-500 mx-1">|</span>
                  <span>Set</span>
                  <DebouncedWorkspaceInput value={currentRow['SET Name']} onChange={(val) => handleFieldUpdate('SET Name', val)} className="w-6 bg-transparent text-center border-b border-transparent hover:border-violet-500 focus:border-violet-400 outline-none text-violet-300 uppercase" />
                </div>
              </div>
              ...
              <DebouncedWorkspaceTextarea className="w-full bg-black/20 border border-slate-700/60 rounded-xl p-4 text-slate-200 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/50 mb-4 shadow-inner" rows="4"
                      ...
                      <div className="flex items-center space-x-3 relative">
                        ...
                        <div className="flex-1 relative flex items-center">
                          ...
                          <DebouncedWorkspaceInput type="text" value={currentRow[`Option${num} (Mandatory)`]}
                            ...
                            className={`w-full bg-black/20 border rounded-lg py-2.5 pl-9 pr-20 focus:outline-none transition-all ${isCorrect ? 'border-emerald-500/30 text-emerald-100 bg-emerald-900/10' : 'border-slate-800 text-slate-300 focus:border-violet-500/50'}`} />
...
const DebouncedCell = React.memo(({ value: initialValue, onChange, rowData, viewMode, column, isEdited, onTriggerCrop, onImageClick, jobId, onVerifyClick }) => {
      ...
      <div className={`p-2 w-full h-full flex flex-col items-center justify-center min-h-[44px] max-h-[80px] relative group border border-dashed border-transparent hover:border-slate-300 rounded ${bgClass}`}>
        ...
        {isUploading ? <Loader2 className="w-4 h-4 animate-spin text-blue-500" /> : value ? (
          <div className="flex flex-col items-center w-full gap-1">
            ...
            {!viewMode && <label className="text-[9px] text-blue-500 cursor-pointer hover:underline opacity-0 group-hover:opacity-100">Replace<input type="file" accept="image/*" className="hidden" onChange={handleManualUpload} /></label>}
          </div>
        ) : (
          !viewMode && (
    ...
    <div className={`p-2 w-full h-full min-h-[40px] cursor-text break-words whitespace-pre-wrap relative group text-slate-700 max-w-[420px] overflow-hidden ${bgClass}`}
      ...
      {!viewMode && column.includes('text') && (
        <button onClick={(e) => { e.stopPropagation(); onVerifyClick(rowData); }}
          className="absolute bottom-1 right-1 p-1.5 bg-violet-600 text-white rounded-lg opacity-0 group-hover:opacity-100 shadow-lg hover:bg-violet-700" title="Open Workspace">
  ...
  return <textarea className="p-2 w-full h-full min-h-[80px] min-w-[200px] border-2 border-violet-400 focus:ring-4 focus:ring-violet-400/20 focus:outline-none rounded resize-y bg-white text-slate-900 relative z-50 shadow-xl" value={value} onChange={(e) => setValue(e.target.value)} onBlur={onBlur} autoFocus />;
}, (prevProps, nextProps) => {
...
const MemoizedDataRow = React.memo(({ row, rowIndex, visibleColumns, viewMode, activeTab, jobId, editedCells, insertRow, deleteRow, handleDataChange, setCropModalConfig, setImagePreviewConfig, openReviewAtId, highlightedRowId }) => {
  ...
  const hasCorrect = ['Option1 Is Correct?', 'Option2 Is Correct?', 'Option3 Is Correct?', 'Option4 Is Correct?'].some(c => row[c] === 'Yes');
  const isDuplicateTarget = (numOpts > 0 && !hasCorrect) || row['Duplicate_Flag'] === 'Yes';
  useEffect(() => { if (isHighlighted && rowRef.current) rowRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, [isHighlighted]);
  let rowBg = 'hover:bg-violet-50/50';
  ...
  return (
    <tr ref={rowRef} className={`group transition-colors duration-200 ${rowBg} ${isHighlighted ? 'ring-2 ring-inset ring-violet-400' : ''}`}>
      <td className="sticky left-0 bg-white group-hover:bg-violet-50/50 z-20 px-1 py-2 border-b border-r border-slate-200 w-16 align-middle">
        <div className="flex flex-col items-center gap-1 opacity-20 group-hover:opacity-100">
          <button onClick={() => insertRow(row.id, 'above')} className="p-0.5 hover:bg-violet-100 rounded text-violet-500"><Plus className="w-3 h-3" /></button>
          <button onClick={() => deleteRow(row.id)} className="p-1 hover:bg-red-100 rounded text-red-500"><Trash2 className="w-3 h-3" /></button>
          <button onClick={() => insertRow(row.id, 'below')} className="p-0.5 hover:bg-violet-100 rounded text-violet-500"><Plus className="w-3 h-3" /></button>
        </div>
      </td>
      {visibleColumns.map((col) => {
        const isSlNo = col === 'Sl.No';
        return (
          <td key={col} className={`p-0 border-b border-r border-slate-200 align-middle ${isSlNo ? 'sticky left-[64px] min-w-[60px] text-center bg-white group-hover:bg-violet-50/50 z-10' : ''}`}>
...
export default function LanguageParser() {
  ...
  const [selectedTargetLanguage, setSelectedTargetLanguage] = useState("Original");
  const [selectedTargetSet, setSelectedTargetSet] = useState("A");
  ...
  const [langMismatchModal, setLangMismatchModal] = useState(null);
  ...
  const lastWorkspaceSaveSkipRef = useRef('');

  useEffect(() => {
    const loadWorkspace = async () => {
      try {
        const res = await axios.get(`${API_BASE}/redis/load`);
        const remoteWorkspace = res.data?.success && res.data?.workspace ? res.data.workspace : {};
        const backupWorkspace = readWorkspaceBackup(WORKSPACE_BACKUP_KEY);
        const workspaceToRestore = shouldUseWorkspaceBackup(remoteWorkspace, backupWorkspace, 'lp')
          ? backupWorkspace
          : remoteWorkspace;

        if (workspaceToRestore && Object.keys(workspaceToRestore).length > 0) {
          const backupRows = workspaceRowCount(backupWorkspace || {}, 'lp');
          const remoteRows = workspaceRowCount(remoteWorkspace || {}, 'lp');
          if (backupRows > remoteRows && workspaceToRestore === backupWorkspace) {
            showToast(`Restored ${backupRows} saved rows instead of ${remoteRows} backend rows.`);
          }

          const { lp_matrices, lp_openTabs, lp_activeTab, lp_editedCells, lp_engineJobId, lp_customCols } = workspaceToRestore;
          const restoredMatrices = lp_matrices && typeof lp_matrices === 'object' ? lp_matrices : {};
          const restoredOpenTabs = Array.isArray(lp_openTabs) ? lp_openTabs : [];
          ...
          const bestTab =
            (lp_activeTab && Array.isArray(restoredMatrices[lp_activeTab]) && restoredMatrices[lp_activeTab].length > 0 && lp_activeTab) ||
            restoredOpenTabs.find(t => Array.isArray(restoredMatrices[t.id]) && restoredMatrices[t.id].length > 0)?.id ||
            (lp_activeTab && restoredOpenTabs.some(t => t.id === lp_activeTab) && lp_activeTab) ||
            restoredOpenTabs[0]?.id ||
            'engine';
          setActiveTab(bestTab);
          if (lp_editedCells) setEditedCells(lp_editedCells);
          if (lp_engineJobId) setEngineJobId(lp_engineJobId);
          if (lp_customCols) setCustomColumns(lp_customCols);
        }
      } catch {
  ...
  const handleNextFilePair = () => {
    ...
    if (nextIdx < queuedPairs.length) {
      ...
      setTimeout(() => startEngine(selectedTargetSet, nextIdx), 500);
    } else {
  ...
  useEffect(() => {
    ...
    const hasPersistableWorkspace =
      openTabs.length > 0 ||
      !!engineJobId ||
      Object.keys(editedCells).length > 0 ||
      customColumns.length > 0 ||
      Object.values(matrices).some(rows => Array.isArray(rows) && rows.length > 0);

    if (!workspaceLoaded || !hasPersistableWorkspace) return;

    const saveTimer = setTimeout(async () => {
      try {
        const workspaceData = {
          lp_activeTab: activeTab, lp_openTabs: openTabs, lp_editedCells: editedCells,
          lp_engineJobId: engineJobId, lp_customCols: customColumns, lp_matrices: matrices
        };
        const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceData, 'lp');
        if (backupSave.skipped) {
          const skipKey = `${workspaceRowCount(workspaceData, 'lp')}:${backupSave.backupRows}`;
          if (lastWorkspaceSaveSkipRef.current !== skipKey) {
            lastWorkspaceSaveSkipRef.current = skipKey;
            showToast(`Kept ${backupSave.backupRows} saved rows; skipped smaller workspace save.`);
          }
          return;
        }
        await axios.post(`${API_BASE}/redis/save`, { workspace: workspaceData });
      } catch (e) { console.error("Failed to sync workspace to Redis", e); }
    }, 2000);
    return () => clearTimeout(saveTimer);
  }, [workspaceLoaded, activeTab, openTabs, matrices, editedCells, engineJobId, customColumns, history]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Array.isArray(dataToSave)) {
        axios.post(`${API_BASE}/sync-db/${targetJobId}`, { parsed_data: dataToSave })
          .then(async () => {
            await importRowsToExtractionReview(targetJobId, dataToSave);
  ...
  useEffect(() => {
    const block = (e) => {
      ...
      if (engineStatus === 'processing') { e.preventDefault(); e.stopPropagation(); }
    };
  ...
  useEffect(() => {
    const warn = (e) => {
      ...
      e.preventDefault();
      e.returnValue = 'Extraction is in progress — leaving will cancel it.';
    };
  ...
  const handleFindReplace = (findText, replaceText, targetColumn, useRegex) => {
    ...
    showToast(`✅ Find & Replace applied to "${targetColumn}"`);
  };
  ...
  const deleteUnansweredDuplicates = () => {
    ...
    if (removedCount > 0) { pushToHistory(activeTab, newData); isDirtyRef.current = true; showToast(`✅ Removed ${removedCount} duplicate/unanswered questions.`); }
  ...
  const bulkMarkOption1 = () => {
    ...
    const newData = data.map(row => {
      ...
      if (!hasCorrect) {
        count++;
        return { ...row, 'Option1 Is Correct?': 'Yes', 'Option2 Is Correct?': 'No', 'Option3 Is Correct?': 'No', 'Option4 Is Correct?': 'No' };
      }
      return row;
    });
    pushToHistory(activeTab, newData); isDirtyRef.current = true;
    setMatrices(prev => { const u = { ...prev, [activeTab]: newData }; latestDataRef.current = u; latestMatricesRef.current = u; return u; });
    showToast(`✅ Bulk Mark: Set Option A as correct for ${count} question(s).`);
  };
  ...
  const deleteRow = useCallback(async (rowId) => {
    ...
    setMatrices(prev => {
      ...
      latestMatricesRef.current = updated;
      return updated;
    });
    ...
    if (activeTabInfo && rowToDelete.id) {
      ...
      try {
        ...
        await importRowsToExtractionReview(activeTabInfo.jobId, newData);
        isDirtyRef.current = false;
      } catch {
        isDirtyRef.current = true;
      }
  ...
  const importRowsToExtractionReview = async (jobId, rows) => {
    if (!jobId || !hasExtractionReviewableRows(rows)) return;

    const signature = buildExtractionReviewImportSignature(jobId, rows);
    if (reviewImportKeysRef.current.has(signature)) return;
    reviewImportKeysRef.current.add(signature);

    try {
      const firstRow = rows.find(row => row?.['Question text(Mandatory)'] || row?.['Question Header']) || rows[0] || {};
      const response = await importExtractionRowsToQBankReview(axios, {
        workflow: 'language',
        jobId,
        setName: 'ALL',
        sourceFileName: firstRow['NIOS Filename'] || 'Extraction Language Parser',
        subjectName: firstRow['Subject Name'],
        subjectCode: firstRow['Subject Code'],
        rows,
      });
      const reviewId = response.data?.review?.id;
      if (reviewId) showToast(`Review queue updated: #${reviewId}`);
    } catch (error) {
      reviewImportKeysRef.current.delete(signature);
      if (![401, 403].includes(error?.response?.status)) {
        console.warn('Failed to stage extraction rows for extraction review.', error);
      }
    }
  };
    ...
    try {
      ...
      eventSourceRef.current.onmessage = (event) => {
        ...
        if (log.type === "system_control" && (log.text === "ALL_DONE" || log.text === "SET_COMPLETE")) {
          ...
          else setEngineStatus("completed");
        ...
        } else if (log.type === "language_mismatch") {
          setLangMismatchModal(log.data);
        } else if (log.type === "language_confirmed") {
          setLangMismatchModal(null);
          setEngineLogs(prev => [...prev, { type: 'success', text: `✅ Language confirmed: ${log.data.language} — extraction starting...` }]);
        } else {
      ...
      Object.keys(groupedData).forEach(setName => {
        ...
        if (!newMatrices[uniqueTabId]) {
          ...
          groupedData[setName].forEach(incomingRow => {
            ...
            if (existingIndex === -1) {
              existingData.push(incomingRow);
            } else {
              const hasLocalEdits = Object.keys(editedCells).some(key => key.startsWith(`${uniqueTabId}-${incomingRow.id}`));
              if (!hasLocalEdits) existingData[existingIndex] = { ...incomingRow, 'Is_Verified': existingData[existingIndex]['Is_Verified'] || 'No' };
            }
          });
          newMatrices[uniqueTabId] = existingData;
        }
      });
      setMatrices(newMatrices);
      setOpenTabs(newOpenTabs);
      setHistory(newHistory);
      const currentHasRows = Array.isArray(newMatrices[activeTab]) && newMatrices[activeTab].length > 0;
      const nextActiveTab = switchToTab && serverData.length > 0 && (activeTab === 'engine' || !currentHasRows)
        ? lastTabId
        : activeTab;
      const workspaceData = {
        lp_activeTab: nextActiveTab,
        lp_openTabs: newOpenTabs,
        lp_editedCells: editedCells,
        lp_engineJobId: id,
        lp_customCols: customColumns,
        lp_matrices: newMatrices
      };
      const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceData, 'lp');
      if (!backupSave.skipped) {
        try { await axios.post(`${API_BASE}/redis/save`, { workspace: workspaceData }); } catch { }
      } else {
        showToast(`Kept ${backupSave.backupRows} saved rows; skipped smaller backend refresh.`);
      }
      if (markCompleted) setEngineStatus("completed");
      if (switchToTab && serverData.length > 0) {
        setActiveTab(prevTab => {
          const previousHasRows = Array.isArray(newMatrices[prevTab]) && newMatrices[prevTab].length > 0;
          if (prevTab === 'engine' || !previousHasRows) return lastTabId;
          if (markCompleted) showToast(`Extraction Complete! New data ready in tabs.`);
          return prevTab;
        });
      }
      void importRowsToExtractionReview(id, serverData);
  ...
  const exportData = async () => {
    ...
    try {
      ...
      link.href = url; link.setAttribute('download', `Verified_SET_${activeTabInfo.setName}_${activeTabInfo.jobId.substring(0, 6)}.zip`);
      document.body.appendChild(link); link.click(); link.parentNode.removeChild(link);
    } catch { showToast('⚠️ Export failed. Please try again.'); }
    finally { setIsDownloading(false); }
  };

  const clearWorkspace = () => {
    clearWorkspaceBackup(WORKSPACE_BACKUP_KEY);
    setEngineFiles([]); setCurrentQueueIdx(0); setAutoTimer(null);
  ...
  const confirmLanguage = async (useWhich) => {
    if (!langMismatchModal || !engineJobId) return;
    const language = useWhich === 'selected' ? langMismatchModal.selected : langMismatchModal.detected;
    try {
      await axios.post(`${API_BASE}/confirm-language/${engineJobId}`, { language });
      setLangMismatchModal(null);
      setEngineLogs(prev => [...prev, { type: 'info', text: `   ↳ You chose: ${language} — resuming extraction...` }]);
    } catch (e) { console.error("Failed to confirm language", e); }
  };
  ...
  const renderMiniMap = () => {
    ...
    for (let i = 1; i <= enginePageMap.total; i++) {
      ...
      if (pStatus === 'scanning') statusColor = "bg-violet-400 border-violet-300 animate-pulse shadow-[0_0_10px_rgba(167,139,250,0.6)]";
  ...
  return (
    <div className="h-screen w-full bg-slate-50 flex flex-col font-sans text-slate-800 overflow-hidden relative selection:bg-violet-200" onClick={() => setIsVerifyOpen(false)}>
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm flex-shrink-0 z-50">
        <div className="flex items-center gap-4 min-w-0 flex-1">
          <div className="flex items-center gap-3 pr-4 border-r border-slate-200 shrink-0">
            <div className="bg-violet-600 p-1.5 rounded-lg shadow-sm"><Globe className="w-5 h-5 text-white" /></div>
            <div>
              <h1 className="text-lg font-extrabold leading-tight text-slate-800">LanguageParser</h1>
              <p className="text-[10px] text-violet-600 font-bold tracking-widest uppercase">LANGUAGE ENGINE</p>
            </div>
          </div>
          <button onClick={() => {
          ...
          <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 bg-violet-50 rounded-lg border border-violet-200 shrink-0 mr-2">
            <Database className="w-3 h-3 text-violet-500" />
            <span className="text-xs font-bold text-violet-600">Windows: <span className="text-violet-800 font-black">{openTabs.length}</span></span>
          </div>
          <div className="flex gap-1 mt-1 overflow-x-auto custom-scrollbar flex-nowrap items-end pb-1 flex-1 scroll-smooth">
            <button onClick={() => setActiveTab('engine')} className={`shrink-0 whitespace-nowrap px-4 py-2 rounded-t-lg text-sm font-bold border-b-2 transition-all ${activeTab === 'engine' ? 'bg-violet-50 border-violet-600 text-violet-700' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}>
              <div className="flex items-center gap-2">Language Engine {engineStatus === 'processing' && <Loader2 className="w-3.5 h-3.5 animate-spin text-violet-600" />}</div>
            </button>
            {openTabs.map(tab => {
              const p = tabProgress[tab.id] || { verified: 0, total: 0 };
              return (
                <div key={tab.id} title={`QP File: ${tab.qpName || 'Document'}`}
                  className={`shrink-0 whitespace-nowrap flex items-center px-4 py-2 rounded-t-lg border-b-2 cursor-pointer transition-all ${activeTab === tab.id ? 'bg-violet-50 border-violet-600 text-violet-700' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}
                  ...
                  {editingTabId === tab.id
                    ? <input autoFocus type="text" value={editingTabName} onChange={(e) => setEditingTabName(e.target.value)}
                      ...
                      className="text-sm font-bold mr-3 bg-white text-slate-800 border border-violet-400 rounded px-1 outline-none w-32 shadow-inner"
                      onClick={(e) => e.stopPropagation()} />
                    : <span className="text-sm font-bold mr-2 hover:text-violet-500" onDoubleClick={(e) => { e.stopPropagation(); setEditingTabId(tab.id); setEditingTabName(tab.label); }}>{tab.label}</span>
                  }
          ...
          <button onClick={async () => {
            ...
            if (dataToSync.length > 0) {
              try {
                ...
                await importRowsToExtractionReview(jid, dataToSync);
              } catch { }
            }
            try {
              const workspaceSnap = { lp_activeTab: activeTab, lp_openTabs: openTabs, lp_editedCells: editedCells, lp_engineJobId: engineJobId, lp_customCols: customColumns, lp_matrices: latestMatricesRef.current };
              const backupSave = saveWorkspaceBackup(WORKSPACE_BACKUP_KEY, workspaceSnap, 'lp');
              if (!backupSave.skipped) {
                await axios.post(`${API_BASE}/redis/save`, { workspace: workspaceSnap });
              } else {
                showToast(`Kept ${backupSave.backupRows} saved rows; skipped smaller sync.`);
              }
            } catch { }
            fetchFinalData(jid, currentQueueIdx);
          }} className="px-4 py-2 bg-violet-50 border border-violet-200 hover:bg-violet-100 text-violet-700 rounded-lg text-sm font-bold shadow-sm flex items-center gap-2">
        ...
        <div className={`flex flex-col bg-slate-50 shadow-[4px_0_20px_rgba(0,0,0,0.05)] min-h-0 z-20 transition-all duration-500 ease-in-out overflow-hidden ${activeTab !== 'engine' ? "w-0 opacity-0 border-r-0" : "w-full opacity-100"}`}>
          <div className="p-4 gap-4 flex flex-col h-full overflow-y-auto custom-scrollbar max-w-3xl mx-auto w-full">
            <div className="flex-shrink-0 bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              <div className="relative border-2 border-dashed border-slate-300 rounded-xl p-6 text-center bg-slate-50 hover:bg-violet-50 hover:border-violet-400 transition-all cursor-pointer group">
                ...
                <Upload className="w-6 h-6 text-violet-500 mx-auto mb-2 group-hover:scale-110 transition-all" />
                <p className="text-xs font-bold text-slate-500">Drop QP + MS PDFs Here</p>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <div className="flex items-center justify-between bg-slate-100 p-2 rounded border border-slate-200">
                  <label className="text-xs font-bold text-slate-600">Paper Language:</label>
                  <select value={selectedTargetLanguage} onChange={(e) => setSelectedTargetLanguage(e.target.value)}
                    ...
                    <option value="Original">Auto-detect (I'm not sure)</option>
                    {SUPPORTED_LANGUAGES.filter(l => l.value !== 'Original').map(l => (
                      <option key={l.value} value={l.value}>{l.label}</option>
                    ))}
                  </select>
                </div>
                <div className="flex items-center justify-between bg-slate-100 p-2 rounded border border-slate-200">
                  <label className="text-xs font-bold text-slate-600">Target Set:</label>
              ...
              {engineJobId && (
                <div className="mt-2 text-xs font-bold text-violet-700 bg-violet-50 p-2 rounded border border-violet-200 flex items-center justify-between shadow-sm">
                  ...
                  <span className="text-violet-900">{jobHeartbeat.rows} row(s) - {jobHeartbeat.message}</span>
                </div>
              )}
            </div>
            {crossValidation && (
              ...
              <div className="flex gap-2 w-full flex-col">
                {autoTimer !== null && (
                  <div className="w-full bg-amber-50 border border-amber-200 rounded-xl p-3 flex justify-between items-center mb-2 shadow-sm">
                    <div className="flex items-center gap-2 text-amber-700 font-bold">
                      ...
                      <span>Next document in: <span className="text-xl mx-1">{Math.floor(autoTimer / 60)}:{(autoTimer % 60).toString().padStart(2, '0')}</span></span>
                    </div>
                ...
                <div className="flex gap-2 w-full">
                  {engineStatus === "idle" || engineStatus === "error" ? (
                    <button onClick={() => startEngine(selectedTargetSet, currentQueueIdx)} disabled={queuedPairs.length === 0}
                      className="flex-1 py-3 bg-violet-600 text-white rounded-xl font-bold hover:bg-violet-700 disabled:bg-slate-300 flex items-center justify-center gap-2 shadow-md">
                      <Play className="w-4 h-4 fill-current" /> Run Language Extraction
                    </button>
                  ) : engineStatus === "completed" ? (
                    <button onClick={() => startEngine(selectedTargetSet, currentQueueIdx)} className="flex-1 py-3 bg-violet-100 border border-violet-200 text-violet-700 rounded-xl font-bold hover:bg-violet-200 shadow-sm flex items-center justify-center gap-2">
                      <Play className="w-4 h-4 fill-current" /> Re-Run Extraction
                    </button>
                  ) : (
                    <>
                  ...
                  <button onClick={resetEngine} className="px-4 py-3 bg-white text-slate-500 hover:bg-red-50 hover:text-red-600 rounded-xl font-bold shadow-sm flex items-center justify-center border border-slate-200">
            ...
            <div className="flex-1 flex flex-col bg-black rounded-xl border border-slate-800 shadow-xl overflow-hidden relative min-h-[250px] shrink-0">
              {engineStatus === "processing" && <div className="absolute top-0 left-0 h-[3px] bg-violet-400 z-50 transition-all duration-500 shadow-[0_0_10px_rgba(167,139,250,0.8)]" style={{ width: `${engineProgress}%` }}></div>}
              <div className="bg-black px-4 py-2.5 border-b border-slate-800 flex justify-between items-center shadow-md z-10 flex-shrink-0 mt-1">
                <span className="font-bold text-slate-400 tracking-widest text-[10px] font-mono flex items-center gap-2"><Activity className="w-3 h-3 text-violet-400 animate-pulse" /> LANGUAGE X-RAY TUNNEL</span>
                {engineStatus === "processing" && <span className="text-violet-400 font-bold text-[10px]">{engineProgress}%</span>}
              </div>
              {renderMiniMap()}
              ...
              {activeTab !== 'engine' && (
                <>
                  ...
                  <div className="flex gap-2">
                    <button onClick={undo} disabled={!history[activeTab] || history[activeTab].past.length === 0} className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-violet-600 hover:bg-violet-50 disabled:opacity-40 shadow-sm"><Undo className="w-4 h-4" /></button>
                    <button onClick={redo} disabled={!history[activeTab] || history[activeTab].future.length === 0} className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-violet-600 hover:bg-violet-50 disabled:opacity-40 shadow-sm"><Redo className="w-4 h-4" /></button>
                  </div>
                </>
              )}
            </div>
              ...
              {activeTab !== 'engine' && (
                <>
                  <div className="inline-flex items-center gap-1 bg-slate-100 border border-slate-200 p-1 rounded-lg">
                    {[['list', 'List'], ['verify', 'Verify'], ['full', 'Grid']].map(([m, l]) => (
                      <button key={m} onClick={() => setColumnMode(m)}
                        className={`px-2.5 py-1 text-xs font-bold rounded transition-colors ${columnMode === m ? 'bg-white text-violet-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                  ...
                  <button onClick={() => setShowUnverifiedOnly(v => !v)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border transition-all ${showUnverifiedOnly ? 'bg-violet-100 border-violet-300 text-violet-700' : 'bg-white border-slate-200 text-slate-500 hover:border-violet-300 hover:text-violet-600'}`}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setIsVerifyOpen(true); }}
          ...
          <div className="flex-1 overflow-auto custom-scrollbar bg-white border border-slate-200 rounded-xl shadow-sm min-h-0 relative">
            {activeTab === 'engine' ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400">
                <Globe className="w-14 h-14 mb-4 opacity-50" />
                <p className="font-bold text-lg">Language Engine Dashboard Active</p>
                <p className="text-sm text-slate-400 mt-1">
                  {selectedTargetLanguage === 'Original'
                    ? 'Language: Auto-detect'
                    : <>Pre-declared: <span className="text-violet-400 font-bold">{selectedTargetLanguage}</span></>
                  }
                </p>
              </div>
            ) : activeData.length === 0 ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 gap-3">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-violet-50 text-violet-700 border border-violet-200 rounded-lg font-bold text-xs hover:bg-violet-100"
                >
                  Reload from backend
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl flex flex-col">
            <div className="flex justify-between items-center mb-6 border-b border-slate-200 pb-4">
              <h2 className="text-xl font-bold flex items-center gap-2 text-slate-800"><FolderUp className="w-6 h-6 text-violet-600" /> Restore Session</h2>
            ...
            <div className="relative border-2 border-dashed border-slate-300 rounded-xl p-8 text-center bg-slate-50 hover:bg-violet-50/50 hover:border-violet-400 cursor-pointer group mb-6">
              ...
              <Upload className="w-8 h-8 text-violet-500 mx-auto mb-3 group-hover:scale-110" />
            ...
            <div className="flex justify-end gap-3 mt-auto border-t border-slate-200 pt-4">
              ...
              <button onClick={executeWorkspaceRestore} disabled={isImporting || !importFiles.find(f => f.name.endsWith('.xlsx') || f.name.endsWith('.xls'))}
                className="px-8 py-2.5 bg-violet-600 text-white font-bold rounded-lg hover:bg-violet-500 disabled:opacity-50 flex items-center gap-2 shadow-md">
      ...
      {langMismatchModal && (
        <div className="fixed inset-0 z-[800] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border-2 border-amber-200 overflow-hidden animate-fade-in">
            <div className="bg-amber-50 border-b border-amber-200 px-7 py-5 flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-amber-100 border-2 border-amber-300 flex items-center justify-center flex-shrink-0">
                <AlertTriangle className="w-5 h-5 text-amber-600" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-800">Language Mismatch Detected</h2>
                <p className="text-xs text-amber-700 font-medium mt-0.5">Extraction is paused — please confirm which language to use</p>
              </div>
            </div>
            <div className="px-7 py-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-violet-50 border-2 border-violet-200 rounded-xl p-4 text-center">
                  <p className="text-[10px] font-black text-violet-500 uppercase tracking-widest mb-2">You Selected</p>
                  <p className="text-2xl font-black text-violet-700 mb-1">{langMismatchModal.selected}</p>
                  <p className="text-xs text-violet-500">your pre-declaration</p>
                </div>
                <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4 text-center">
                  <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2">AI Detected</p>
                  <p className="text-2xl font-black text-amber-700 mb-1">{langMismatchModal.detected}</p>
                  <p className="text-xs text-amber-500">from cover page scan</p>
                </div>
              </div>
              <p className="text-sm text-slate-500 text-center mb-6">These do not match. Which language should the engine use for extraction?</p>
              <div className="space-y-3">
                <button onClick={() => confirmLanguage('selected')}
                  className="w-full py-3.5 bg-violet-600 hover:bg-violet-500 text-white font-bold rounded-xl transition-all shadow-sm flex items-center justify-between px-5">
                  <span>Use <strong>{langMismatchModal.selected}</strong></span>
                  <span className="text-violet-200 text-xs font-normal">my selection — I know what I uploaded</span>
                </button>
                <button onClick={() => confirmLanguage('detected')}
                  className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 text-white font-bold rounded-xl transition-all shadow-sm flex items-center justify-between px-5">
                  <span>Use <strong>{langMismatchModal.detected}</strong></span>
                  <span className="text-amber-100 text-xs font-normal">AI detected — trust the scan</span>
                </button>
              </div>
              <p className="text-[11px] text-slate-400 text-center mt-5">
                If no selection is made within 5 minutes, your pre-declared language ({langMismatchModal.selected}) will be used automatically.
              </p>
            </div>
          </div>
        </div>
