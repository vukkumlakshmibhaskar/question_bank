...
const SUPPORTED_LANGUAGES = [
  ...
  { value: 'Odiya', label: 'Odiya (à¬“à¬¡à¬¼à¬¿à¬†)' },
...
// âœ… PATCH 2: Language â†’ script/direction mapping for the /scan-text endpoint.
// Internal config â€” never shown to the user.
...
const TextScannerModal = ({ isOpen, onClose, rowData, columnToUpdate, jobId, onScanComplete }) => {
  ...
  // âœ… PATCH 3: handleScan now calls the backend /scan-text endpoint instead of Tesseract.
...
export default function LanguageParser() {
    ...
    try {
      ...
      eventSourceRef.current.onmessage = (event) => {
        ...
        if (log.type === "system_control" && (log.text === "ALL_DONE" || log.text === "SET_COMPLETE")) {
          ...
          setEngineLogs(prev => [...prev, { type: 'success', text: `âœ… Language confirmed: ${log.data.language} â€” extraction starting...` }]);
        } else {
  ...
  const exportData = async () => {
    ...
    } catch { showToast('âš ï¸ Export failed. Please try again.'); }
  ...
  const confirmLanguage = async (useWhich) => {
    ...
    try {
      ...
      setLangMismatchModal(null);
      setEngineLogs(prev => [...prev, { type: 'info', text: `   â†³ You chose: ${language} â€” resuming extraction...` }]);
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm flex-shrink-0 z-50">
        ...
        <div className="flex items-center gap-3 shrink-0 ml-4">
          <button onClick={async () => {
            ...
            if (dataToSync.length > 0) { try { await axios.post(`${API_BASE}/sync-db/${jid}`, { parsed_data: dataToSync }); } catch { } }
            try {
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border-2 border-amber-200 overflow-hidden animate-fade-in">
            <div className="bg-amber-50 border-b border-amber-200 px-7 py-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 font-medium mt-0.5">Extraction is paused â€” please confirm which language to use</p>
              </div>
            </div>
            <div className="px-7 py-6">
              ...
              <div className="space-y-3">
                <button onClick={() => confirmLanguage('selected')}
                  ...
                  <span className="text-violet-200 text-xs font-normal">my selection â€” I know what I uploaded</span>
                </button>
                <button onClick={() => confirmLanguage('detected')}
                  ...
                  <span className="text-amber-100 text-xs font-normal">AI detected â€” trust the scan</span>
                </button>
              </div>
