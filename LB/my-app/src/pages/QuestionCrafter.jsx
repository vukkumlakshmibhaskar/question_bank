...
const DEFAULT_QUESTION_CRAFTER_API_BASE = window.location.protocol === 'https:'
  ? '/question-crafter-api'
  : 'http://192.168.1.234:8100';
                ...
                return (
                  ...
                  >
                    <div className="flex h-40 items-center justify-center bg-white">
                      <img src={apiUrl(cropUrl)} alt={`Possible image ${index + 1}`} className="max-h-40 w-full object-contain" />
                    </div>
...
function ReviewModal({ open, rows, index, sessionId, sourcePages = [], onClose, onSave, onNavigate, onRegenerate }) {
  ...
  const saveCrop = async () => {
    ...
    setCropError('');
    try {
      const response = await fetch(apiUrl('/crop-source-image'), {
        method: 'POST',
                      ...
                      return (
                        ...
                        >
                          <div className="flex h-28 items-center justify-center bg-white">
                            <img src={apiUrl(cropUrl)} alt={`Image choice ${cropIndex + 1}`} className="max-h-28 w-full object-contain" />
                          </div>
            ...
            {canAttachImage && draft.source_image_url ? (
              <div className="rounded-lg border border-cyan-800 bg-cyan-950/30 p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <a className="text-xs font-bold text-cyan-200 underline" href={apiUrl(draft.source_image_url)} target="_blank" rel="noreferrer">Open image</a>
                </div>
                {cropMode ? (
                  <ReactCrop crop={cropSelection} onChange={(_, percentCrop) => setCropSelection(percentCrop)} aspect={undefined}>
                    <img src={apiUrl(draft.source_image_url)} alt="Source page" className="max-h-[58vh] min-h-[360px] w-full rounded border border-cyan-900 object-contain bg-white" />
                  </ReactCrop>
                ) : (
                  <img src={apiUrl(draft.source_image_url)} alt="Source page" className="max-h-[58vh] min-h-[360px] w-full rounded border border-cyan-900 object-contain bg-white" />
                )}
...
export default function QuestionCrafter() {
  ...
  useEffect(() => {
    ...
    const checkApi = async () => {
      try {
        const response = await fetch(apiUrl('/gateway-health'), { cache: 'no-store' });
  ...
  const analyzeTextbook = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/detect-lessons'), { method: 'POST', body: formData });
      const data = await response.json();
  ...
  const generateLesson = async (lesson) => {
    ...
    setError('');
    try {
      const response = await fetch(apiUrl('/generate-lesson'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const generateMissingAnswers = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl('/generate-missing-answers'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: textbookSessionId,
  ...
  const exportReviewed = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl('/export-reviewed'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const downloadResponse = await fetch(`${apiUrl(data.download_url)}?t=${Date.now()}`);
  ...
  const regenerateRow = async (index, row) => {
    ...
    try {
      const lessonConfig = getLessonSettings(activeLesson);
      const response = await fetch(apiUrl('/regenerate-row'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: textbookSessionId,
                          ...
                          {isVisualRow(row) && (row.source_page || row.source_image_url) && (
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-cyan-700">
                              ...
                              {imageFileName(row.source_image_url)
                                ? <a className="font-bold underline" href={apiUrl(row.source_image_url)} target="_blank" rel="noreferrer">question image</a>
