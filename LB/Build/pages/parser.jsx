  ...
  const handleScan = async () => {
    ...
    try {
      ...
      Tesseract.recognize(canvas, 'eng').then(({ data: { text } }) => {
        onScanComplete(text, rowData.id, columnToUpdate);
        setIsScanning(false);
        onClose();
      });
    } catch (e) { alert("OCR Engine Failed."); setIsScanning(false); }
  };
    ...
    const loadWorkspace = async () => {
      try {
        ...
        if (res.data && res.data.success && res.data.workspace) {
          ...
          if (dp_matrices && Object.keys(dp_matrices).length > 0) setMatrices(dp_matrices);
          if (dp_openTabs && dp_openTabs.length > 0) setOpenTabs(dp_openTabs);
          if (dp_activeTab) setActiveTab(dp_activeTab);
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm flex-shrink-0 z-50">
        <div className="flex items-center gap-4 min-w-0 flex-1">
          ...
          <button onClick={() => {
            ...
            navigate('/language');
          }} className="px-4 py-1.5 bg-violet-100 text-violet-700 hover:bg-violet-200 font-bold text-xs rounded-lg border border-violet-200 shadow-sm ml-2">
            🔄 Switch to Language Parser
          </button>
            ...
            <div className="flex-shrink-0 bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              ...
              {queuedPairs.length > 0 && (
                <div className="mt-2 text-xs font-bold text-emerald-700 bg-emerald-50 p-2 rounded border border-emerald-200 flex items-center justify-between shadow-sm">
                  ...
                  {engineStatus === 'processing' && <span className="flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Processing Pair {currentQueueIdx + 1}</span>}
                </div>
              )}
            </div>
                ...
                <div className="flex gap-2 w-full">
                  {engineStatus === "idle" || engineStatus === "error" ? (
                    <button onClick={() => startEngine("A", currentQueueIdx)} disabled={queuedPairs.length === 0 || currentQueueIdx >= queuedPairs.length}
                      ...
                      <Play className="w-4 h-4 fill-current" /> Run Queue (Pair {currentQueueIdx + 1})
                    </button>
