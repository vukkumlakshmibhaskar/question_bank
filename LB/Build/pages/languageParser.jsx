...
import {
  ...
  Replace, BarChart2, Columns, Filter,
...
const TextScannerModal = ({ isOpen, onClose, rowData, columnToUpdate, jobId, onScanComplete }) => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      Tesseract.recognize(canvas, 'eng').then(({ data: { text } }) => { onScanComplete(text, rowData.id, columnToUpdate); setIsScanning(false); onClose(); });
    } catch { alert("OCR Engine Failed."); setIsScanning(false); }
  };
  return (
      ...
      <div className="bg-[#0f172a] rounded-xl shadow-2xl border border-cyan-700 w-full max-w-6xl flex flex-col max-h-[95vh]">
        ...
        <div className="px-6 py-4 border-t border-slate-700 bg-slate-900/50 flex justify-end gap-3 flex-shrink-0">
          ...
          <button onClick={handleScan} disabled={!completedCrop || completedCrop.width === 0 || isScanning}
            ...
            {isScanning ? <Loader2 className="w-5 h-5 animate-spin" /> : <ScanText className="w-5 h-5" />} Extract Text
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div onClick={e => e.stopPropagation()} className={`fixed inset-y-0 right-0 w-96 bg-white/95 backdrop-blur-2xl border-l border-slate-200 shadow-[0_0_50px_rgba(0,0,0,0.2)] z-[60] transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
        <h2 className="text-lg font-bold text-slate-800 flex items-center"><span className="mr-2 text-xl">🛡️</span> Verification</h2>
...
// ==========================================
// REVIEW MODAL — all keyboard/navigation bugs fixed
// ==========================================
function ReviewModal({ isOpen, initialRowId, data, onClose, onUpdate, jobId, onImagePreview, onTriggerScanner, insertRow, deleteRow, flaggedIds }) {
  ...
  // FIX 3a: pendingNavigateId — waits for state to commit before navigating
  ...
  // FIX 2b: keep ref in sync with state
  ...
  // Collects all unique page numbers from data, used by PageList for the page strip.
  ...
  useEffect(() => {
    ...
    // MS mode: do NOT auto-scroll — MS_Page_Image_URL is a distributed placeholder,
  ...
  // FIX 2c: keyboard handler uses currentIndexRef — never stale
  // FIX 2d: currentIndex and currentRow removed from dependency array
  useEffect(() => {
    ...
    // FIX 2d: no currentIndex / currentRow in deps
        ...
        <div className={`px-4 py-2 border-b flex-shrink-0 ${isVerified ? 'border-emerald-700 bg-emerald-950/30' : 'border-slate-800 bg-slate-900/50'}`}>
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button onClick={() => {
                if (currentRow) {
                  // FIX 3d: pendingNavigateId for Add button
        ...
        <div className="flex flex-1 overflow-hidden bg-[#0B1120]/50">
          ...
          <div ref={rightPanelRef} className="w-5/12 p-7 overflow-y-auto bg-transparent flex flex-col custom-scrollbar gap-4">
            <div className="text-[10px] text-slate-600 font-mono bg-black/20 px-3 py-1.5 rounded border border-slate-800 flex-shrink-0">
              ←→ navigate · 1-4 options · V verify · J next issue · I insert · C crop · D dup · Esc close
            </div>
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => onTriggerScanner('Question text(Mandatory)', 'scan', currentRow)} className="flex items-center gap-1 px-2 py-1 bg-cyan-900/30 hover:bg-cyan-800 text-cyan-400 text-xs rounded border border-cyan-800">
                  <ScanText className="w-3.5 h-3.5" /> Scan Text
                </button>
              </div>
            ...
            <div>
              <div className="bg-black/30 border border-slate-800 rounded-xl p-5 shadow-inner">
                ...
                <div className="flex space-x-3 mb-4">
                  <button onClick={() => onTriggerScanner('If Question is Image, Specify Image Name', 'crop_qp', currentRow)} className="flex-1 bg-slate-800/50 hover:bg-slate-700/80 text-slate-300 font-medium py-2.5 rounded-lg border border-slate-700/50 text-sm flex justify-center items-center gap-2">📐 Map QP Crop</button>
                  <button onClick={() => onTriggerScanner('Answer_Diagram_Image', 'crop_ms', currentRow)} className="flex-1 bg-fuchsia-900/30 hover:bg-fuchsia-800/40 text-fuchsia-300 font-medium py-2.5 rounded-lg border border-fuchsia-700/50 text-sm flex justify-center items-center gap-2">📐 Map MS Crop</button>
                </div>
...
const DebouncedCell = React.memo(({ value: initialValue, onChange, rowData, viewMode, column, isEdited, onTriggerCrop, onImageClick, jobId, onVerifyClick }) => {
  ...
  if (viewMode || !isEditing) return (
    // FIX 4: max-w-[420px] overflow-hidden prevents Tamil/long-text column stretching
...
// ==========================================
// MAIN LANGUAGE PARSER COMPONENT
// ==========================================
export default function LanguageParser() {
  ...
  const queuedPairs = useMemo(() => {
    const pairs = [], unmatchedQPs = {}, unmatchedMSs = {};
    engineFiles.forEach(f => {
      const isQP = f.name.toLowerCase().startsWith('qp');
      const isMS = f.name.toLowerCase().startsWith('ms');
      let baseName = f.name.replace(/^(qp|ms)_/i, '').replace(/_[a-zA-Z0-9]{15,}\.pdf$/i, '').replace(/\.pdf$/i, '');
      if (isQP) {
        if (unmatchedMSs[baseName]?.length > 0) { pairs.push({ baseName, qp: f, ms: unmatchedMSs[baseName].shift() }); }
        else { if (!unmatchedQPs[baseName]) unmatchedQPs[baseName] = []; unmatchedQPs[baseName].push(f); }
      } else if (isMS) {
        if (unmatchedQPs[baseName]?.length > 0) { pairs.push({ baseName, qp: unmatchedQPs[baseName].shift(), ms: f }); }
        else { if (!unmatchedMSs[baseName]) unmatchedMSs[baseName] = []; unmatchedMSs[baseName].push(f); }
      }
    });
    return pairs;
  }, [engineFiles]);
  ...
  const [langMismatchModal, setLangMismatchModal] = useState(null); // {selected, detected}
  ...
  // FIX: tracks rows explicitly deleted by the user — fetchFinalData will never re-add them
  const deletedRowIdsRef = useRef(new Set());

  // FIX 1a: refs for always-fresh state in fetchFinalData
    ...
    const loadWorkspace = async () => {
      try {
        ...
        if (res.data && res.data.success && res.data.workspace) {
          const { lp_matrices, lp_openTabs, lp_activeTab, lp_editedCells, lp_engineJobId, lp_customCols } = res.data.workspace;
          if (lp_matrices && Object.keys(lp_matrices).length > 0) setMatrices(lp_matrices);
          if (lp_openTabs && lp_openTabs.length > 0) setOpenTabs(lp_openTabs);
          if (lp_activeTab) setActiveTab(lp_activeTab);
      ...
      } catch { console.warn("Could not load from Redis, starting fresh."); }
    };
    loadWorkspace();
  }, []);
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api-status`).then(res => setApiKeyStatus(`${res.data.current} / ${res.data.total}`)).catch(() => setApiKeyStatus("Offline"));
  }, []);

  useEffect(() => {
    if (engineJobId && Object.keys(matrices).length === 0) fetchFinalData(engineJobId, currentQueueIdx);
  }, []);

  // FIX 1b: sync refs — fetchFinalData reads from refs not stale closure
  useEffect(() => {
    ...
    const saveTimer = setTimeout(async () => {
      try {
        const workspaceData = {
          lp_activeTab: activeTab,
          lp_openTabs: openTabs,
          lp_editedCells: editedCells,
          lp_engineJobId: engineJobId,
  ...
  // ── SAFETY NET 1: Block backspace browser-back during extraction ──────────
  useEffect(() => {
    const block = (e) => {
  ...
  // ── SAFETY NET 2: Warn before tab close / refresh during extraction ───────
  useEffect(() => {
    const warn = (e) => {
  ...
  const startEngine = async (targetSet = "A", queueIdx = currentQueueIdx) => {
    ...
    try {
      ...
      formData.append("files", queuedPairs[queueIdx].ms);
      ...
      eventSourceRef.current = new EventSource(`${API_BASE}/logs/${activeJobId}`);
      eventSourceRef.current.onmessage = (event) => {
        const log = JSON.parse(event.data);
        if (log.type === "system_control" && (log.text === "ALL_DONE" || log.text === "SET_COMPLETE")) {
          eventSourceRef.current.close();
          // Set status immediately so spinner stops — don't wait for fetchFinalData
          if (log.text === "ALL_DONE") setEngineStatus("idle");
          else setEngineStatus("completed"); // SET_COMPLETE → show "Continue to next set"
          // FIX 1c: uses refs internally
          ...
          // Pause UI — show confirmation modal
          setLangMismatchModal(log.data);
        } else if (log.type === "language_confirmed") {
          // Match confirmed — close modal if open, log result
          setLangMismatchModal(null);
          setEngineLogs(prev => [...prev, {
            type: 'success',
            text: `✅ Language confirmed: ${log.data.language} — extraction starting...`
          }]);
        } else {
  ...
  // FIX 1d: fetchFinalData reads refs — avoids stale closure
  const fetchFinalData = async (id, queueIdx) => {
    try {
      let serverData = [];
      try {
        ...
        // /data/{job_id} 404 — job state missing (server restart, Redis expiry, etc.)
        // Fall back to workspace: any data already in our matrices is preserved.
        // Nothing to merge from server, just keep current state.
      ...
      serverData.forEach(row => {
        ...
        groupedData[s].push({ ...row, id: row.id || uuidv4(), 'Is_Verified': row['Is_Verified'] || 'No' });
      });
      Object.keys(groupedData).forEach(setName => {
        ...
        if (!newMatrices[uniqueTabId]) {
          ...
          groupedData[setName].forEach(incomingRow => {
            ...
            if (deletedRowIdsRef.current.has(incomingRow.id)) {
              // Also remove it from existingData in case stale latestMatricesRef snuck it in
              const staleIdx = existingData.findIndex(r => r.id === incomingRow.id);
      ...
      setActiveTab(prevTab => {
        if (prevTab === 'engine') return lastTabId;
        showToast(`Extraction Complete! New data ready in tabs.`);
        return prevTab;
      });
    } catch { }
  };
  ...
  // Called when user resolves a language mismatch in the confirmation modal
  const confirmLanguage = async (useWhich) => {
    if (!langMismatchModal || !engineJobId) return;
    const language = useWhich === 'selected'
      ? langMismatchModal.selected
      : langMismatchModal.detected;
    try {
      await axios.post(`${API_BASE}/confirm-language/${engineJobId}`, { language });
      setLangMismatchModal(null);
      setEngineLogs(prev => [...prev, {
        type: 'info',
        text: `   ↳ You chose: ${language} — resuming extraction...`
      }]);
    } catch (e) {
      console.error("Failed to confirm language", e);
    }
  };
    ...
    <div className="h-screen w-full bg-slate-50 flex flex-col font-sans text-slate-800 overflow-hidden relative selection:bg-violet-200" onClick={() => setIsVerifyOpen(false)}>
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm flex-shrink-0 z-50">
        <div className="flex items-center gap-4 min-w-0 flex-1">
          ...
          <button onClick={() => {
            if (engineStatus === 'processing' && !window.confirm('Extraction is running. Switch parser anyway?')) return;
            navigate('/standard');
          }} className="px-4 py-1.5 bg-blue-100 text-blue-700 hover:bg-blue-200 font-bold text-xs rounded-lg border border-blue-200 shadow-sm ml-2">
            🔄 Switch to Standard Parser
          </button>
          <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg border border-slate-200 shrink-0">
            ...
            <span className="text-xs font-bold text-slate-500">API Key: <span className="text-slate-800">{apiKeyStatus}</span></span>
          </div>
          {toast && (
          ...
          <button onClick={async () => {
            ...
            try {
              const workspaceSnap = {
                ...
                lp_engineJobId: engineJobId,
                lp_customCols: customColumns,
                lp_matrices: latestMatricesRef.current
              };
            ...
            <div className="flex-shrink-0 bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              ...
              {queuedPairs.length > 0 && (
                <div className="mt-2 text-xs font-bold text-emerald-700 bg-emerald-50 p-2 rounded border border-emerald-200 flex items-center justify-between shadow-sm">
                  <span>✅ {queuedPairs.length} Complete Pair(s) Ready</span>
                  {engineStatus === 'processing' && <span className="flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Processing...</span>}
                </div>
              )}
            </div>
                ...
                {autoTimer !== null && (
                  <div className="w-full bg-amber-50 border border-amber-200 rounded-xl p-3 flex justify-between items-center mb-2 shadow-sm">
                    <div className="flex items-center gap-2 text-amber-700 font-bold">
                      ...
                      <span>Next Pair in: <span className="text-xl mx-1">{Math.floor(autoTimer / 60)}:{(autoTimer % 60).toString().padStart(2, '0')}</span></span>
                    </div>
      ...
      {/* ── LANGUAGE MISMATCH CONFIRMATION MODAL ─────────────────────── */}
      {langMismatchModal && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border-2 border-amber-200 overflow-hidden animate-fade-in">
            ...
            <div className="px-7 py-6">
              ...
              <p className="text-sm text-slate-500 text-center mb-6">
                These do not match. Which language should the engine use for extraction?
              </p>
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => confirmLanguage('detected')}
